...
import dict_from_bestsellers


logreg = LogisticRegression(random_state=16, max_iter=200)
logreg_1 = LogisticRegression(random_state=16, max_iter=200)
...
list_bestsellers_words = dict_from_bestsellers.dictionary_from_bestsellers('bestsellers.csv', 70)
pattern = '|'.join(list_bestsellers_words)
...
for chunk in pd.read_csv('train.csv', chunksize = 100000):
    df = chunk[['title', 'reviews', 'boughtInLastMonth', 'price', 'categoryName', 'isBestSeller']].copy()

    # get a df without bestsellers
    df_intermediate = df.loc[df['isBestSeller'] == False]

    df_no_bs = df_intermediate[['title', 'reviews', 'boughtInLastMonth', 'price', 'categoryName', 'isBestSeller']].copy()
    df_with_bs = bestsellers[['title', 'reviews', 'boughtInLastMonth', 'price', 'categoryName', 'isBestSeller']].copy()

    df_no_bs['reviewNormalized'] = (df_no_bs['reviews'] - mean_)/rev_stdev
    df_with_bs['reviewNormalized'] = (df_with_bs['reviews'] - mean_)/rev_stdev

    df_no_bs['ohe_title'] = 0
    df_no_bs['ohe_title'] = df_no_bs.title.str.contains(pattern)

    df_with_bs['ohe_title'] = 1
    ...
    feature_cols = ['reviewNormalized', 'boughtInLastMonth', 'price', 'categoryMedianPrice', 'ohe_title']
...
df_test_intermed = df_test[['reviews', 'boughtInLastMonth', 'price', 'categoryName', 'isBestSeller', 'title']].copy()
df_test_intermed['reviewNormalized'] = (df_test_intermed['reviews'] - mean_)/rev_stdev

df_test_intermed['ohe_title'] = df_test_intermed.title.str.contains(pattern)
